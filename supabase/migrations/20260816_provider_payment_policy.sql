-- Require CarDaddy's no-advance-payment policy for every new provider application.

alter table public.provider_applications
  add column if not exists no_advance_fee_acknowledged boolean not null default false;

alter table public.provider_profiles
  add column if not exists no_advance_fee_acknowledged boolean not null default false;

alter table public.provider_applications
  alter column minimum_mobilization_fee set default null;
...
begin
  ...
  if p_status = 'Approved' then
    insert into public.provider_profiles (
      ...
      availability_start_date, emergency_available, no_advance_fee_acknowledged, account_status
    )
    select id, full_name, business_name, phone, email, state, city, zip_code,
      max_travel_radius, max_travel_hours, languages, specialties, vehicle_types_served,
      vehicle_types_not_served, services_offered, services_not_offered,
      availability_start_date, emergency_available, no_advance_fee_acknowledged, 'Active'
    ...
    on conflict (application_id) do update set
      ...
      no_advance_fee_acknowledged = excluded.no_advance_fee_acknowledged,
      updated_at = now();
  end if;
