...
set street_address = trim(approximate_location)
where nullif(trim(street_address), '') is null;

alter table public.service_cases
  alter column street_address set not null;

comment on column public.service_cases.street_address is
  'Exact physical street address supplied by the customer for mobile service dispatch and future geocoding.';
...
declare
  ...
  service_address text := coalesce(
    nullif(trim(p_payload->>'street_address'), ''),
    nullif(trim(p_payload->>'approximate_location'), '')
  );
begin
  if coalesce(trim(p_payload->>'customer_name'), '') = ''
    ...
    or service_address is null
    or coalesce(trim(p_payload->>'problem_description'), '') = '' then
    raise exception 'Missing required service request information';
  end if;
  if service_address !~ '[0-9]' or service_address !~* '[a-z]' or service_address ~* '\mP\.?\s*O\.?\s+Box\M' then
    raise exception 'A complete physical service address is required';
  end if;
  ...
  insert into public.service_cases (
    ...
    service_address, p_payload->>'state', trim(p_payload->>'city'), trim(p_payload->>'zip_code'), service_address,
